...
bool Magician::attack() {
  cout << "* 마법사의 공격은 마나가 있을 시 마나를 일정 소모해" <<
    "2배의 데미지를, 마나가 없을 시 0.5배의 데미지를 줍니다." << endl;
  return false;
}
bool Magician::attack(Monster* monster, string& debugs) {
  ...
  outputs += "* 마법사 " + nickname + "의 " +
    to_string(damage) + "데미지 공격!\n";
  outputs += "* 몬스터 " + monster->getName() + "의 남은 체력: "
    + to_string(monster->getHP()) + "\n";
  if (!hpCheck) {
    outputs += "\n";
