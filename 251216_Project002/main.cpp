...
string menuItems() // used on menu text input
{
	string outputs = "";
	outputs += "1. 전투 시작\n";
	outputs += "esc. 종료\n";
...
void battleTask(CurrentMenu& menuStats, Player& plGet, string& battleMsg) // used on thread
{
	menuStats = CurrentMenu::inBattle;

	string randNames[] = { "슬라임", "고블린", "오크", "트롤", "스켈레톤" };
	int randIndex = rand() % 5;
	Monster monster(randNames[randIndex]);
...
int main()
{
	const string ANSI_CURSOR_HOME = "\033[H"; // Move cursor to home position
	const string ANSI_CLEAR_SCREEN = "\033[J"; // Clear screen from cursor down
    const string ANSI_CURSOR_HIDE = "\033[?25l"; // Hide the cursor
    const string ANSI_CURSOR_SHOW = "\033[?25h"; // Show the cursor
    ...
    while (currentMenu != CurrentMenu::dead && currentMenu != CurrentMenu::exitCommand)
    {
        ...
        if (_kbhit())
        {
            ...
            switch (currentMenu)
            {
                case mainMenu:
                    ...
                    {
                        battleMsg = "";
                        thread battleThread(battleTask, ref(currentMenu), ref(*player), ref(battleMsg));
                        battleThread.detach();
                    }
                    break;
                case inBattle:
    ...
    {
        cout << "* 플레이어 " + player->getNickname() + "가(이) 사망했습니다. 게임을 종료합니다." << endl;
    }
    else
    {
